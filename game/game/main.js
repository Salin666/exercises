...
class Field {
  ...
  playGame() {
    ...
    while (currentlyPlaying) {
      moveSelection = prompt("Which way would you like to move? (Up = 'w' Down = 's' Left = 'a' Right = 'd'");

      if (moveSelection.toUpperCase() === 'w' && y !== 0) {
        y -= 1;
      } else if (moveSelection.toUpperCase() === 'w' && y === 0) {
      ...
      } else if (moveSelection.toUpperCase() === 'a' && x !== 0) {
        x -= 1;
      } else if (moveSelection.toUpperCase() === 'a' && x === 0) {
        console.log('You have already reached the left side of the board');
      } else if (moveSelection.toUpperCase() === 'd' && x !== (inputWidth - 1)) {
        x += 1;
      } else if (moveSelection.toUpperCase() === 'd' && x === (inputWidth - 1)) {
...
let inputHeight = prompt('How tall');
